...
def get_avro_schema(file_path: Path) -> dict:
    """avsc 파일을 읽어 avro 스키마로 변환합니다."""
    try:
        with open(file_path, "r") as f:
            schema_json = json.load(f)
            avro_schema = parse_schema(schema_json)
            logger.info(f"Avro schema({file_path}) loaded successfully.")
            return avro_schema
...
def serialize_to_avro(message: dict, schema: dict) -> bytes:
...
master_fields = set(field["name"] for field in master_schema["fields"])
